...
description: |
  ...
  **元数据强制**：每个 H5 必须带 `> 优先级: 高/中/低 | 关联需求: ... | 前置条件: ... | 测试数据: ...`。
tools:
  - Bash
  - Read
  - Write
...
**在生成任何用例之前**，必须先读以下文件，确保用例用真实业务术语和上下文：

1. **`knowledge/README.md`** — 知识库入口和主题分类
2. **`knowledge/glossary.md`** — 业务术语表（**用词必须与本表一致**）
3. **`knowledge/关键场景速查.md`** — 根据当前需求涉及的功能定位知识库路径
4. **按需深入** `knowledge/_raw/小二当家/...`（具体路径见关键场景速查.md）
...
**不要预先建索引**。在收到需求时，**实时**调用按需索引脚本：

```bash
python scripts/build-on-demand-index.py \
  --need "<用户需求描述>" \
  --keywords "<关键词1>,<关键词2>,..." \
  --output knowledge/_index/按需/{需求名}-{时间戳}.md
```
...
按 `.trae/templates/testcase-spec.md` 中"测试点文档"规范，输出到：
```
testpoints/{需求名称}-测试点-{序号}.md
```
...
按 `.trae/templates/testcase-spec.md` 中"测试用例文档"规范，输出到：
```
testcases/{需求名称}-测试用例-{序号}.md
```

**关键**：
- H5 用例场景后**必须**加元数据块 `> 优先级: 高/中/低 | 关联需求: REQ_X | 前置条件: ... | 测试数据: ...`
- 用例中出现的业务术语（客户类型、页面名称、模块名等）必须与 `knowledge/glossary.md` 一致
- 用例涉及的关联组件、上下游业务关系必须参考 `knowledge/_raw/...` 知识库
...
### 阶段 8：输出报告
向用户报告：拉取来源、3 个产物路径、关键统计（测试点/用例数）。
...
- 不要修改 `.trae/templates/testcase-spec.md` 和本文件
